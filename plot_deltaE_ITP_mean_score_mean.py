...
plt.scatter(e_means_flat[valid_indices], score_means_flat[valid_indices], c=point_color_flat[valid_indices], edgecolors=point_color_flat[valid_indices])
for indice in range(valid_indices.size):
    if not valid_indices[indice]:
        continue
    # elif e_means_flat[indice] > 5:
    #     continue
    plt.text(e_means_flat[indice], score_means_flat[indice] - 0.02,
...
correlation_coefficient_Pearson = pearsonr(e_means_flat[valid_indices], score_means_flat[valid_indices]).correlation
correlation_coefficient_Kendall = kendalltau(e_means_flat[valid_indices], score_means_flat[valid_indices]).correlation
correlation_coefficient_Spearman = spearmanr(e_means_flat[valid_indices], score_means_flat[valid_indices]).correlation
...
plt.xlim([0, 15])
plt.xlabel('deltaE 2000 (Mean of 1000 values)')
plt.ylabel('Score (Mean of 74 values)')
plt.title('74 Subject Scores and 1000 CMFs deltaE 2000 Mean points')
