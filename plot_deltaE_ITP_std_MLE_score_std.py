...
MLE_data = pd.read_csv(r'E:\About_Cambridge\All Research Projects\Lab_code_submmit\individual-color-metamerism/MLE_CSV.csv')
...
for i in range(6):
    for j in range(11):
        if all(score == 0 for score in score_cell[i, j]):
            ...
            query_condition_id = f'D1_{display_patterns[i, 0]}-D2_{display_patterns[i, 1]}-C_{j}'
            row_idx = MLE_data['Condition_ID'] == query_condition_id
            MLE_std[i, j] = MLE_data.MLE_STD_Score[row_idx]
...
plt.scatter(e_std_flat[valid_indices], MLE_std_flat[valid_indices], c=point_color_flat[valid_indices], edgecolors=point_color_flat[valid_indices])
for indice in range(valid_indices.size):
    if not valid_indices[indice]:
        continue
    # elif e_means_flat[indice] > 5:
    #     continue
    plt.text(e_std_flat[indice], MLE_std_flat[indice] - 0.02,
...
correlation_coefficient_Pearson = pearsonr(e_std_flat[valid_indices], MLE_std_flat[valid_indices]).correlation
correlation_coefficient_Kendall = kendalltau(e_std_flat[valid_indices], MLE_std_flat[valid_indices]).correlation
correlation_coefficient_Spearman = spearmanr(e_std_flat[valid_indices], MLE_std_flat[valid_indices]).correlation
...
plt.title('74 Subject Scores MLE and 1000 CMFs deltaE 2000 Standard deviation points')
